...
import sign_up_bg from "../assert/images/sign_in/sign_up_bg.png";
	...
	signIn = ()=>{
		...
		LC.User.logInWithMobilePhoneSmsCode('+86'+phoneNumber, code)
			.then((user) => {
				...
				setTimeout(()=>{
					console.log('管理系统登录成功')
					IMClass?.createIMClient(user, ()=>{
						this.props.enqueueSnackbar(
							'登录成功',
							{variant: 'success' }
						);
						this.props.router.navigate('/manager');
					});
				}, 1000)
			}, (e) => {
				// 验证码不正确
		...
		return (
			<Spin
				...
				<div style={{
					...
					minHeight: '100vh',
					flexDirection: 'column',
					background: `url("${sign_up_bg}") center center / contain repeat`,
				}}>
					{this.renderStack()}
					<Container
						style={{
						...
						<Container
							...
							<FormText style={{color: 'black', fontWeight: 'bold', fontSize: 16}}>登录</FormText>
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(Sign_in));
