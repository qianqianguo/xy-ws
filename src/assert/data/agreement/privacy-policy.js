...
Youla平台（以下简称“本平台”或“我们”）由重庆月下童信息科技有限公司提供服务，我们非常重视用户的隐私。您在使用我们的服务时，我们会收集和使用您的相关个人信息及用户信息（以下统称为“信息”）。我们希望通过《Youla隐私政策》（以下简称《隐私政策》或“本政策”）向您说明，您在使用我们的服务时，我们如何收集、使用、储存、共享、转移、披露这些信息，以及您可以通过本文所列途径及方式查阅、复制、更正、补充、删除你的个人信息，也可以撤回同意协议、注销账号、投诉举报以及设置个性化内容的隐私功能。
...
（5）身高、学校、收入、职业、个人介绍、兴趣爱好等。您使用Youla交友服务时，我们会收集一些您额外的个人信息，以为您提供更个性化的服务。额外的个人信息包括身高、收入、职业、个人介绍、兴趣爱好。因Youla匹配的交友模式，您提供的额外个人信息会被其他用户查阅。若您选择不提供上述额外的个人信息，不影响您使用Youla的基础服务；
...
（10）充值订单信息。我们会收集您在使用Youla服务过程中产生的充值订单信息便于对账号上购买的增值服务进行管理。充值订单信息属于个人敏感信息，收集上述信息是为了实现Youla消费功能，否则将无法完成交易；

（11）读取手机状态和身份权限。为了识别用户身份，保障账号信息安全和用户聊天交流，我们会申请发现已知账户、读取手机状态和身份权限，保存您的账号信息、账号、口令、昵称、头像；

（12）网络访问日志信息。为了满足相关法律法规要求和网络安全保障需要，我们会保存您的网络访问日志信息：IP地址、用户登录时间、用户退出时间；

（13）通信/通话记录和内容。当您联系我们的客服或使用其他用户响应功能时（个人信息保护投诉或建议、未成年人相关投诉或建议或其他客户投诉和需求），我们会保存您的联系方式（您与我们联系时使用的或您向我们主动提供的其他联系方式）、您与我们的通信/通话记录和内容、与您需求相关联的其他必要信息，以便与您联系或帮助您解决问题，或记录相关问题的处理方案及结果；

（14）剪切板信息。当您使用Youla服务时，我们会使用您设备上的剪切板功能（如有），识别剪切板的内容，以便您进行身份认证、复制聊天信息。

（15）获取应用列表。基于安全风控的目的，我们需要获取您的应用列表；

（16）为了解您的产品适配性、识别账号异常状态，我们会收集关于您使用的服务以及使用方式的信息并将这些信息进行关联，这些信息包括：

日志信息：当您使用我们的服务时，我们会自动收集您对我们服务的详细使用情况，作为有关网络日志保存。包括您的登录账号、IP地址、运营商信息、网络环境、WIFI状态信息。

设备信息：为了保障移动互联网应用业务安全风控，应对反作弊、反欺诈、违法不良信息管控等安全风险，我们可能会保存您的设备信息，同时根据您在软件安装及使用中授予的具体权限，接收并记录您所使用的设备相关信息（包括可能的设备唯一标识符、设备硬件序列号、Android ID、IMSI、国际移动设备身份码IMEI、 网络设备硬件MAC地址、广告标识符IDFA、设备机型、操作系统及版本、设备安装软件列表、应用包名、客户端版本、设备分辨率、设备设置、SIM卡信息、CPU信息、内存信息、电池使用情况）、BSSID信息、设备所在位置相关信息（包括IP地址、GPS位置以及能够提供相关信息的WLAN接入点、蓝牙和基站传感器信息）。
...
（17）授权共享信息。我们会从关联方、第三方合作伙伴获取您授权共享的相关信息。包括我们从第三方获取您授权共享的账户信息（包括用户名、头像、昵称）并在您同意本政策后将您的第三方账户与您的Youla服务账户绑定，使您可以通过第三方账户直接登录并使用我们的产品与/或服务。我们将在符合相关法律法规规定，并依据与关联方或第三方合作伙伴的约定、确信其提供的信息来源合法的前提下，收集并使用您的这些信息。若你提供的信息中含有其他用户的个人信息，在向本平台提供这些个人信息之前，您需确保您已经取得合法的授权。
...
8、您理解并同意，部分单项服务可能需要您在您的设备中开启特定的访问权限（包括您的位置信息 、麦克风、相机和存储），以实现这些权限所涉及信息的收集和使用。当您需要关闭该功能时，大多数移动设备都会支持您的这项需求，具体方法请参考或联系您移动设备的服务商或生产商。请您注意，您开启任一权限即代表您授权我们可以收集和使用相关信息来为您提供对应服务，您一旦关闭任一权限即代表您取消了授权，我们将不再基于对应权限继续收集和使用相关信息，也无法为您提供该权限所对应的服务。但是，您关闭权限的决定不会影响此前基于您的授权所进行的信息收集及使用。
...
重庆月下童信息科技有限公司将按照本政策保护您的个人信息，如您有关于个人信息安全的投诉和举报，或您对本政策、您的信息的相关事宜有任何问题、意见或建议，请随时与我们联系。可发送邮件至moonxt@yeah.net。
